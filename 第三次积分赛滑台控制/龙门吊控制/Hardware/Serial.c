...
char Serial3_RxPacket[100];	//定义接收数据包数组，数据包格式"@MSG\r\n"
uint8_t Serial3_RxFlag;		//定义串口接收的标志位变量
...
{
	/*开启时钟*/
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_USART3, ENABLE);	//开启USART3的时钟,注意函数名！
...
void USART3_IRQHandler(void)
{
	...
	if (USART_GetITStatus(USART3, USART_IT_RXNE) == SET)	//判断是否是USART3的接收事件触发的中断
	{
		...
		if (RxState == 0)
		{
			if (RxData == '@' && Serial3_RxFlag == 0)		//如果数据确实是包头，并且上一个数据包已处理完毕
			{
				RxState = 1;			//置下一个状态
		...
		else if (RxState == 2)
		{
			...
			{
				RxState = 0;			//状态归0
				Serial3_RxPacket[pRxPacket] = '\0';			//将收到的字符数据包添加一个字符串结束标志
